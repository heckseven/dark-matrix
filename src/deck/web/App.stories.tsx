...
import { App } from './App';
import { deckStore, ROWS, DEFAULT_WIDTH } from './store.js';
...
function setup(frames: Frame[], opts: {
  ...
  deckStore.getState().loadProject({ frames, width: W, mode: opts.mode ?? 'bw', loop: true });
  deckStore.setState({ isPlaying: opts.isPlaying ?? false, zoom: opts.zoom ?? 1 });
  deckStore.getState().setActiveMode(opts.activeMode !== undefined ? opts.activeMode : 'design');
  if (opts.activeFrameIdx !== undefined) deckStore.setState({ activeFrameIdx: opts.activeFrameIdx });
  if (opts.previewTarget !== undefined) deckStore.getState().setPreviewTarget(opts.previewTarget);
}

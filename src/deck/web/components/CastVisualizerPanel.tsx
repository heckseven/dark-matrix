 ...
 */
export function CastVisualizerPanel({ open, onOpenChange, dualModule, hasMic }: {
  open: boolean;
  ...
  const apply = useCallback(() => {
    ...
    if (sel === 'off') {
      ...
      ws.send(JSON.stringify({ type: 'audio-viz', style: sel, source: src }));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        // Only render grid previews while the picker is visible — the modules keep
        // running regardless, but no one can see the tiles when it's closed.
        if (msg.type === 'audio-bands' && Array.isArray(msg.bands) && openRef.current) {
          const ctx: RenderCtx = {
            bands: msg.bands,
            fftSize: typeof msg.fftSize === 'number' ? msg.fftSize : 2048,
            gain: typeof msg.gain === 'number' ? msg.gain : 1.0,
          };

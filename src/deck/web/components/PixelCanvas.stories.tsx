...
function resetStore(width: 9 | 18) {
  deckStore.getState().loadProject({
...
export const UndoRedo: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const before = deckStore.getState().frames[0]!.pixels;
    deckStore.getState().setPixel(0, 0, 0, 255);
    const painted = deckStore.getState().frames[0]!.pixels;
    await expect(painted).not.toBe(before);

    container.focus();
    await userEvent.keyboard('{Control>}z{/Control}');
    await expect(deckStore.getState().frames[0]!.pixels).toBe(before);

    await userEvent.keyboard('{Control>}y{/Control}');
    await expect(deckStore.getState().frames[0]!.pixels).toBe(painted);
  },
};

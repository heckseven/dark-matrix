...
function syncFrameCount(target: number) {
  ...
  for (let n = deckStore.getState().frames.length; n < t; n++) {
    deckStore.getState().addFrame(n - 1);
  }
  for (let n = deckStore.getState().frames.length; n > t; n--) {
    deckStore.getState().removeFrame(n - 1);
  }
}
...
function PlaybackStory({ frameCount }: { frameCount: number }) {
  useEffect(() => {
    ...
    deckStore.getState().setActiveFrame(0);
    deckStore.getState().setPlaying(false);
  }, [frameCount]);
  return <Playback />;
}
...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component: 'Playback controls: previous, play/pause, next, and frame counter. Connects to the deck store.',
      },
    },
  },
  argTypes: {

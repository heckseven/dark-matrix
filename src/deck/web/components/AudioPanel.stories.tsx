...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Audio visualizer panel. Manages its own WebSocket connection for live FFT frame streaming. Source (monitor/mic) and style are persisted in the deck store.',
      },
    },
  },
  args: {
...
export const MicSource: Story = {
  decorators: [
    (Story) => {
      deckStore.setState({ audioSource: 'mic' });
...
export const WaterfallStyle: Story = {
  decorators: [
    (Story) => {
      deckStore.setState({ audioStyle: 'waterfall' });
...
export const HeatStyle: Story = {
  decorators: [
    (Story) => {
      deckStore.setState({ audioStyle: 'heat' });
...
export const SelectStyle: Story = {
  ...
  play: async ({ canvas }) => {
    ...
    await expect(deckStore.getState().audioStyle).toBe('waterfall');
  },
};

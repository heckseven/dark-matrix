    ...
    <TooltipProvider>
      ...
      <div ref={containerRef} aria-busy={configLoading} className="relative h-screen bg-background text-foreground font-mono">
        <PanelBar
          ...
          style={{ backdropFilter: 'blur(2px)', backgroundColor: 'color-mix(in srgb, var(--color-background) 65%, transparent)', ...(activeMode === 'video' ? idleFadeStyle(videoIdle) : activeMode === 'audio' && audioFullscreenStyle !== null ? idleFadeStyle(audioIdle) : {}) }}
          left={
        ...
        {(!isFullscreenMode(activeMode) || activeMode === 'life') && <footer ref={footerRef} role="contentinfo" aria-label={activeMode === 'life' ? 'Simulation status' : 'Editor status'} className="absolute bottom-0 inset-x-0 z-10 flex items-center px-7 py-4 text-xs" style={{ backdropFilter: 'blur(2px)', backgroundColor: 'color-mix(in srgb, var(--color-background) 65%, transparent)' }}>
          {activeMode === 'life' ? (
            <>

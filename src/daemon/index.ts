...
import { createTextRenderer, TEXT_STYLES, TEXT_SIZES, TEXT_SPEEDS, TEXT_FLICKERS } from '../animations/text-renderers.js';
        ...
        for (const line of lines) {
          ...
          switch (msg.cmd) {
            ...
            case 'hud-config': {
              ...
              const buildText = (text: string, st?: string, sz?: string, sp?: string, span?: boolean, fl?: string): NonNullable<NonNullable<Config['hud']>['left']> => {
                const style = asTextStyle(st); const size = asTextSize(sz); const speed = asTextSpeed(sp); const flicker = asTextFlicker(fl);
                return { widget: 'text', text: text.slice(0, 128), ...(style ? { style } : {}), ...(size ? { size } : {}), ...(speed ? { speed } : {}), ...(span ? { span: true } : {}), ...(flicker ? { flicker } : {}) };
              };
              ...
              if (m.leftWidget === 'audio') {
                ...
                newHud.left = buildText(m.leftText, m.leftTextStyle, m.leftTextSize, m.leftTextSpeed, m.leftTextSpan, m.leftTextFlicker);
              ...
              if (m.rightWidget === 'audio') {
                ...
                newHud.right = buildText(m.rightText, m.rightTextStyle, m.rightTextSize, m.rightTextSpeed, m.rightTextSpan, m.rightTextFlicker);

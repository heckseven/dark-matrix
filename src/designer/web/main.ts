...
previewBtn.addEventListener('click', () => {
  if (bridge) {
    ...
    unsub = store.subscribe(() => {
      const { frames, activeFrameIdx, mode, width, previewTarget, previewBw } = store.state;
      const frame = frames[activeFrameIdx];
      if (frame && bridge) bridge.sendFrame(frame.pixels, previewBw ? 'bw' : mode, width, previewTarget);
    });

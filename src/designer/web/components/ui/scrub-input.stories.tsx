    ...
    docs: {
      description: {
        component: [
          ...
          '**Usage**',
          '```tsx',
          '<ScrubInput value={opacity} onChange={setOpacity} min={0} max={100} />',
          '```',
        ].join('\n'),
      },
    },
...
export const ColorChannel: Story = {
  render: args => <Controlled {...args} />,
  args: { value: 128, min: 0, max: 255, className: 'w-8 text-center' },
};

/** When the visible label is narrow, the input expands on focus to show more digits. */
export const ExpandsOnFocus: Story = {
  render: args => <Controlled {...args} />,
  args: { value: 42, min: 0, max: 9999, className: 'w-8 text-center', expandedClassName: 'w-16 text-center' },
};

...
### User permissions
Groups: adm, cdrom, sudo, dip, plugdev, users, lpadmin, vboxusers, kismet, libvirt, ollama — no `audio` or `pipewire` group membership.
...
### Session log format
Log file location: `~/.claude/projects/-home-heckseven-projects-dark-matrix/<session-uuid>.jsonl`
...
### User permissions
Groups: uid=1000(heckseven) gid=1000(heckseven) groups=1000(heckseven),4(adm),24(cdrom),27(sudo),30(dip),46(plugdev),100(users),114(lpadmin),124(vboxusers),126(kismet),128(libvirt),984(ollama)
...
1. **udev rule + ectool** (fast): Add a udev rule granting `heckseven` read access
   to `/dev/cros_ec`, install `ectool`, poll `ectool switches` every ~500ms from
   the daemon. This is a pre-Phase-2 setup step, not a code change.
   ```
   # /etc/udev/rules.d/99-cros-ec-user.rules
   KERNEL=="cros_ec", GROUP="heckseven", MODE="0660"
   ```
